...
public class HighPriorityDataChangeQueueReceiver {
    ...
    public HighPriorityDataChangeQueueReceiver() {
        ThreadFactory namedThreadFactory = new ThreadFactoryBuilder().setNameFormat("HighPriorityDataChangeQueueReceiver").build();
     ...
     * 聚合product消息的去重队列
     */
    private Set<String> dimHighPriorityDataMessageSet = Collections.synchronizedSet(new HashSet<>());

    @RabbitHandler
    public void process(String content) {
        log.info("高优先级商品同步服务消费消息 : " + content);
    ...
    private void processBrandDataChangeMessage(JSONObject jsonObject) {
        ...
        rabbitMQSender.send(RabbitQueue.HIGH_PRIORITY_AGGR_DATA_CHANGE_QUEUE, "{\"dim_type\": \"brand\", \"id\": " + id + "}");
    }
    ...
    private void processCategoryDataChangeMessage(JSONObject jsonObject) {
        ...
        rabbitMQSender.send(RabbitQueue.HIGH_PRIORITY_AGGR_DATA_CHANGE_QUEUE, "{\"dim_type\": \"category\", \"id\": " + id + "}");
    }
    ...
    private void processProductDataChangeMessage(JSONObject jsonObject) {
        ...
        // 使用消息去重队列
        // rabbitMQSender.send(RabbitQueue.HIGH_PRIORITY_AGGR_DATA_CHANGE_QUEUE, "{\"dim_type\": \"product\", \"id\": " + id + "}");
        dimHighPriorityDataMessageSet.add("{\"dim_type\": \"product\", \"id\": " + id + "}");
    }
    ...
    private void processProductIntroDataChangeMessage(JSONObject jsonObject) {
        ...
        rabbitMQSender.send(RabbitQueue.HIGH_PRIORITY_AGGR_DATA_CHANGE_QUEUE, "{\"dim_type\": \"product_intro\", \"id\": " + productId + "}");
    }
    ...
    private void processProductPropertyDataChangeMessage(JSONObject jsonObject) {
        ...
        // 使用消息去重队列
        // rabbitMQSender.send(RabbitQueue.HIGH_PRIORITY_AGGR_DATA_CHANGE_QUEUE, "{\"dim_type\": \"product\", \"id\": " + productId + "}");
        dimHighPriorityDataMessageSet.add("{\"dim_type\": \"product\", \"id\": " + productId + "}");
    }
    ...
    private class SendThread extends Thread {
        ...
        public void run() {
            while (true) {
                if (dimHighPriorityDataMessageSet.size() != 0) {
                    for (String dimDataMessage : dimHighPriorityDataMessageSet) {
                        rabbitMQSender.send(RabbitQueue.HIGH_PRIORITY_AGGR_DATA_CHANGE_QUEUE, dimDataMessage);
                    }
                    dimHighPriorityDataMessageSet.clear();
                }
                try {
                    // 间隔1秒
                    Thread.sleep(1000);
